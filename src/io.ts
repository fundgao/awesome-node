...
export class Io {
  ...
  private initEventHook() {
    ...
    wechaty.on("error", (error) =>
      this.send({ name: "error", payload: error })
    );
    wechaty.on("heartbeat", (data) =>
      this.send({ name: "heartbeat", payload: { cuid: this.id, data } })
    );
    wechaty.on("login", (user) =>
      this.send({
        name: "login",
        payload: wechaty.puppet.contactPayload(user.id),
      })
    );
    ...
    wechaty.on("scan", (qrcode, status) =>
      this.send({ name: "scan", payload: { qrcode, status } } as IoEventScan)
    );
  }
  ...
  private async initWebSocket(): Promise<WebSocket> {
    ...
    ws.on("close", (code, reason) =>
      this.wsOnClose(ws, code, reason.toString())
    );
    ...
    switch (ioEvent.name) {
      ...
      case "reset":
        ...
        this.options.wechaty.emitError(
          new Error("reset by server: " + JSON.stringify(ioEvent.payload))
        );
      ...
      case "jsonrpc":
        ...
        try {
          ...
          const payload = jsonRpcPeer.parse(
            response
          ) as jsonRpcPeer.JsonRpcPayloadResponse;
  ...
  private wsOnError(e?: Error) {
    ...
    if (!this.ws) {
      log.error(
        "Io",
        "wsOnError() ws.on(error) this.ws is `undefined`",
        e.message
      );
      return;
    }
    ...
    if (this.ws.readyState === WebSocket.CONNECTING) {
      log.error(
        "Io",
        "wsOnError() ws.on(error) ws.readyState is CONNECTING: %s",
        e.message
      );
      return;
    }
    ...
    if (this.ws.readyState === WebSocket.CLOSING) {
      log.error(
        "Io",
        "wsOnError() ws.on(error) ws.readyState is CLOSING: %s",
        e.message
      );
      return;
    }
  ...
  private reconnect() {
    ...
    log.warn(
      "Io",
      "reconnect() will reconnect after %d s",
      Math.floor(this.reconnectTimeout / 1000)
    );
  ...
  private async send(ioEvent?: IoEvent): Promise<void> {
    ...
    if (!this.connected()) {
      log.verbose(
        "Io",
        "send() without a connected websocket, eventBuffer.length = %d",
  ...
  private async ioMessage(m: MessageInterface): Promise<void> {
    log.silly(
      "Io",
      "ioMessage() is a nop function before be overwritten from cloud"
    );
