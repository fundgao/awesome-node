...
const MixinBase = wechatifyMixin(
  poolifyMixin(RoomEventEmitter)<RoomImplInterface>()
);
...
class RoomMixin extends MixinBase implements SayableSayer {
  ...
  static async create(
    ...
    try {
      ...
      log.error(
        "Room",
        "create() exception: %s",
        (e && (e as Error).stack) || (e as Error).message || (e as Error)
      );
      throw e;
    }
  }
  ...
  static async findAll(query?: PUPPET.filters.Room): Promise<RoomInterface[]> {
    ...
    try {
      ...
      const idToRoom = async (id: string) =>
        this.wechaty.Room.find({ id }).catch((e) => this.wechaty.emitError(e));
      ...
      const roomIterator =
        concurrencyExecuter(CONCURRENCY)(idToRoom)(roomIdList);
  ...
  static async find(
    ...
    for (const [idx, room] of roomList.entries()) {
      ...
      if (valid) {
        log.verbose(
          "Room",
          "find() room<id=%s> is valid: return it",
          idx,
          room.id
        );
        return room;
        ...
        log.verbose(
          "Room",
          "find() room<id=%s> is invalid: skip it",
          idx,
          room.id
        );
      }
  ...
  async ready(forceSync = false): Promise<void> {
    ...
    const contactIterator =
      concurrencyExecuter(CONCURRENCY)(doReady)(memberIdList);
  ...
  async say(
    ...
    if (isTemplateStringArray(sayable)) {
      ...
      msgId = await deliverSayableConversationPuppet(this.wechaty.puppet)(
        this.id
      )(sayable);
    }
  ...
  private async sayTemplateStringsArray(
    ...
    const mentionList = varList.filter((c) =>
      ContactImpl.valid(c)
    ) as ContactInterface[];
    ...
    if (varList.length === 0) {
      ...
      for (; i < varListLength; i++) {
        if (ContactImpl.valid(varList[i])) {
          ...
          const mentionName =
            (await this.alias(mentionContact)) || mentionContact.name();
  ...
  async del(contact: ContactImpl): Promise<void> {
    log.warn(
      "Room",
      "del() is DEPRECATED, use remove() instead.\n%s",
      new Error().stack
    );
  ...
  async topic(newTopic?: string): Promise<void | string> {
    ...
    if (typeof newTopic === "undefined") {
      if (this.payload && this.payload.topic) {
        ...
        const memberList = (await Promise.all(memberListFuture)).filter(
          Boolean
        ) as ContactInterface[];
  ...
  async memberAll(
    filter: PUPPET.filters.RoomMember
  ...
  async memberAll(
    ...
    const contactIdList = await this.wechaty.puppet.roomMemberSearch(
      this.id,
      query
    );
  ...
  protected async memberList(): Promise<ContactInterface[]> {
    ...
    const contactListAll = await Promise.all(
      memberIdList.map((id) => this.wechaty.Contact.find({ id }))
    );
