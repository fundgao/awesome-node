...
import { packageJson, GIT_COMMIT_HASH } from "./package-json.js";
...
if (log.level() === "verbose" || log.level() === "silly") {
  ...
  process.on("unhandledRejection", (reason: Error | any, promise) => {
    ...
    log.error(
      "Config",
      "Wechaty unhandledRejection: %s %s",
      reason.stack || reason,
      promise
    );
    log.error("Config", "###########################");
    promise.catch((err) => {
      log.error(
        "Config",
        "process.on(unhandledRejection) promise.catch(%s)",
        err.message
      );
...
export class Config {
  ...
  apihost =
    process.env["WECHATY_APIHOST"] || DEFAULT_SETTING["DEFAULT_APIHOST"];
