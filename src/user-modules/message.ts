...
import { log, AT_SEPARATOR_REGEX } from "../config.js";
...
import type { ContactInterface, ContactImpl } from "./contact.js";
import type { RoomInterface, RoomImpl } from "./room.js";
...
import { PostInterface, PostImpl } from "./post.js";
import { LocationInterface, LocationImpl } from "./location.js";
...
class MessageMixin extends MixinBase implements SayableSayer {
  ...
  override toString() {
    ...
    if (
      this.type() === PUPPET.types.Message.Text ||
  ...
  talker(): ContactInterface {
    ...
    if (!talkerId) {
      ...
      log.warn(
        "Message",
        "talker() payload.talkerId not exist! See: https://github.com/wechaty/puppet/issues/187"
      );
      console.error(
        "Puppet: %s@%s",
        this.wechaty.puppet.name(),
        this.wechaty.puppet.version()
      );
    ...
    let talker;
    if (
      this.wechaty.isLoggedIn &&
      talkerId === this.wechaty.puppet.currentUserId
    ) {
      talker = (this.wechaty.ContactSelf as typeof ContactSelfImpl).load(
        talkerId
      );
    } else {
  ...
  from(): undefined | ContactInterface {
    log.warn(
      "Message",
      "from() is deprecated, use talker() instead. Call stack: %s",
      new Error().stack
    );
    try {
      return this.talker();
  ...
  listener(): undefined | ContactInterface {
    ...
    if (listenerId === this.wechaty.puppet.currentUserId) {
      listener = (this.wechaty.ContactSelf as typeof ContactSelfImpl).load(
        listenerId
      );
    } else {
  ...
  async toRecalled(): Promise<undefined | MessageInterface> {
    ...
    try {
      const message = await this.wechaty.Message.find({
      ...
      this.wechaty.emitError(e);
      log.verbose(
        `Can not retrieve the recalled message with id ${originalMessageId}.`
      );
    }
    return undefined;
  }
  ...
  async mentionList(): Promise<ContactInterface[]> {
    ...
    if (
      ...
      const allContact = await Promise.all(
        this.payload.mentionIdList.map(idToContact)
      );
    ...
    const contactListNested = await Promise.all(
      mentionNameList.map((name) => room.memberAll(name))
    );
    ...
    if (contactList.length === 0) {
      log.silly(
        "Message",
        `message.mentionList() can not found member using room.member() from mentionList, mention string: ${JSON.stringify(
          mentionNameList
        )}`
      );
    }
  ...
  async mention(): Promise<ContactInterface[]> {
    log.warn(
      "Message",
      "mention() DEPRECATED. use mentionList() instead. Call stack: %s",
      new Error().stack
    );
  ...
  async ready(): Promise<void> {
    ...
    if (!talkerId) {
      ...
      log.warn(
        "Message",
        "ready() payload.talkerId not exist! See: https://github.com/wechaty/puppet/issues/187"
      );
      console.error(
        "Puppet: %s@%s",
        this.wechaty.puppet.name(),
    ...
    if (!listenerId && this.payload.toId) {
      ...
      log.warn(
        "Message",
        "ready() payload.listenerId should be set! See: https://github.com/wechaty/puppet/issues/187"
      );
      console.error(
        "Puppet: %s@%s",
        this.wechaty.puppet.name(),
   ...
   */
  async forward(
    to: RoomInterface | ContactInterface
  ...
  async toMiniProgram(): Promise<MiniProgramInterface> {
    ...
    const miniProgramPayload = await this.wechaty.puppet.messageMiniProgram(
      this.id
    );
...
class MessageImplBase extends validationMixin(
  MessageMixin
)<MessageImplInterface>() {}
...
class MessageImpl extends validationMixin(
  MessageImplBase
)<MessageInterface>() {}
