...
import { sayableToPayload, payloadToSayableWechaty } from "../sayable/mod.js";
...
class PostBuilder {
  ...
  reply(post: PostInterface): this {
    if (!post.id) {
      throw new Error(
        "can not link to a post without id: " + JSON.stringify(post)
      );
    }
  ...
  async build(): Promise<PostInterface> {
    ...
    this.payload.sayableList = sayablePayloadListNested.filter(
      Boolean
    ) as PUPPET.payloads.Sayable[];
...
class PostMixin extends wechatifyMixinBase() {
  ...
  static async findAll(
    ...
    const { nextPageToken, response: postIdList } =
      await this.wechaty.puppet.postSearch(filter, pagination);

    const idToPost = async (id: string) =>
      this.wechaty.Post.find({ id }).catch((e) => this.wechaty.emitError(e));
  ...
  async *taps(filter: PUPPET.filters.Tap = {}): AsyncIterableIterator<Tap> {
    ...
    while (true) {
      ...
      [tapList, nextPageToken] = await this.tapFind(filter, {
        ...pagination,
        pageToken: nextPageToken,
      });
    }
  ...
  async tapFind(
    ...
    const { nextPageToken, response } = await this.wechaty.puppet.tapSearch(
      this.id,
      filter,
      pagination
    );
