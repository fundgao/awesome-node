...
const payloadToSayableWechaty: (
  w: WechatyInterface
) => (p: PUPPET.payloads.Sayable) => Promise<undefined | Sayable> =
  (wechaty: WechatyInterface) => async (sayable: PUPPET.payloads.Sayable) => {
    log.verbose(
      "Wechaty",
      "payloadToSayable({type: %s(%s)})",
    ...
    switch (sayable.type) {
      ...
      default:
        throw new Error(
          "payloadToSayable() not support payload: " + JSON.stringify(sayable)
        );
    }
  };
