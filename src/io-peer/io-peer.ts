...
): payload is jsonRpcPeer.JsonRpcPayloadRequest => "method" in payload;
const isJsonRpcNotification = (
  payload: jsonRpcPeer.JsonRpcPayload
): payload is jsonRpcPeer.JsonRpcPayloadNotification =>
  isJsonRpcRequest(payload) && !("id" in payload);
const isJsonRpcResponse = (
  payload: jsonRpcPeer.JsonRpcPayload
): payload is jsonRpcPeer.JsonRpcPayloadResponse => "result" in payload;
const isJsonRpcError = (
  payload: jsonRpcPeer.JsonRpcPayload
): payload is jsonRpcPeer.JsonRpcPayloadError => "error" in payload;
...
const getPeer = (options: IoPeerOptions) => {
  ...
  const onMessage = async (
    message: jsonRpcPeer.JsonRpcPayload
  ): Promise<any> => {
