...
class FriendshipMixin extends MixinBase implements Accepter {
  ...
  async accept(): Promise<void> {
    ...
    if (this.payload.type !== PUPPET.types.Friendship.Receive) {
      throw new Error(
        "accept() need type to be FriendshipType.Receive, but it got a " +
          FriendshipImpl.Type[this.payload.type]
      );
    }
    ...
    try {
      ...
      log.warn(
        "Friendship",
        "accept() contact %s not ready because of %s",
        contact,
  ...
  contact(): ContactInterface {
    ...
    const contact = (this.wechaty.Contact as typeof ContactImpl).load(
      this.payload.contactId
    );
    return contact;
  }
  ...
  toJSON(): string {
    ...
    if (!this.isReady()) {
      throw new Error(
        `Friendship<${this.id}> needs to be ready. Please call ready() before toJSON()`
      );
    }
...
class FriendshipImpl extends validationMixin(
  FriendshipMixin
)<FriendshipInterface>() {}
