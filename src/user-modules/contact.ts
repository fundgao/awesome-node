...
const MixinBase = wechatifyMixin(
  poolifyMixin(ContactEventEmitter)<ContactImplInterface>()
);
...
class ContactMixin extends MixinBase implements SayableSayer {
  ...
  static async find(
    ...
    if (typeof query === "object" && query.id) {
      ...
      if (this.wechaty.puppet.currentUserId === query.id) {
         ...
         */
        contact = (
          this.wechaty.ContactSelf as any as typeof ContactSelfImpl
        ).load(query.id);
      } else {
        contact = (this.wechaty.Contact as any as typeof ContactImpl).load(
          query.id
        );
      }
    ...
    if (contactList.length > 1) {
      log.warn(
        "Contact",
        "find() got more than 1 result: %d total",
        contactList.length
      );
    }
    ...
    for (const [idx, contact] of contactList.entries()) {
      ...
      if (valid) {
        log.silly(
          "Contact",
          "find() contact<id=%s> is valid, return it",
          idx,
          contact.id
        );
        return contact;
        ...
        log.silly(
          "Contact",
          "find() contact<id=%s> is invalid, skip it",
          idx,
          contact.id
        );
      }
    ...
    log.warn(
      "Contact",
      "find() all of %d contacts are invalid",
      contactList.length
    );
    return undefined;
  }
  ...
  static async findAll(
    ...
    log.verbose(
      "Contact",
      "findAll(%s)",
      JSON.stringify(query, stringifyFilter) || ""
    );

    try {
      const contactIdList: string[] = await this.wechaty.puppet.contactSearch(
        query
      );

      const idToContact = async (id: string) =>
        this.wechaty.Contact.find({ id }).catch((e) =>
          this.wechaty.emitError(e)
        );
      ...
      const contactIterator =
        concurrencyExecuter(CONCURRENCY)(idToContact)(contactIdList);
      ...
      log.error(
        "Contact",
        "this.wechaty.puppet.contactFindAll() rejected: %s",
        (e as Error).message
      );
      return []; // fail safe
    }
  ...
  override toString(): string {
    ...
    const identity =
      this.payload.alias || this.payload.name || this.id || "loading...";
  ...
  async say(sayable: Sayable): Promise<void | MessageInterface> {
    ...
    const msgId = await deliverSayableConversationPuppet(this.wechaty.puppet)(
      this.id
    )(sayable);
  ...
  async phone(phoneList?: string[]): Promise<string[] | void> {
    log.silly(
      "Contact",
      "phone(%s)",
      phoneList === undefined ? "" : JSON.stringify(phoneList)
    );
...
class ContactImplBase extends validationMixin(
  ContactMixin
...
class ContactImpl extends validationMixin(
  ContactImplBase
)<ContactInterface>() {}
